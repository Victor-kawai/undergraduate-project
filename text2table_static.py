...
LastEditTime: 2024-05-31 16:20:07
...
def data_extract(replace_dict, text_file_name):
    ...
    while text:
        if text.startswith("#### "):
            ...
            if entity != "": # 保存上一个entity
                entity_dict[entity]["参考文档页数"] = entity_dict[entity]["参考文档页数"][:-1]
            ...
            if len(attribute) != 0:
                for key in replace_dict[attribute]:
                    entity_dict[entity][key] += text.replace("\n", "").replace(" ", "")
        text = f.readline()
    entity_dict[entity]["参考文档页数"] = entity_dict[entity]["参考文档页数"][:-1]
    print(entity_dict)
    # 类别	条目名	衙署	下级机构	上级机构	隶属机构	下级官员	平级官员	官品文本	官品	编制文本	编制	编制人数	职掌文本	职掌	简称与别名	参考文档页数	职源与沿革文本	出处	始置时间	罢置时间	前身
...
if __name__ == "__main__":
    text_file = "元丰新制/三省门/三省门文本转换后.md"
